...
var jquery = fs.readFileSync(path.join(__dirname, 'jquery.min.js')).toString()
  ...
  if (!/[<>]/.test(body)) {
    ...
    try {
      ...
      for (var i = 0, l = rows.length; i < l; i += 2) {
        ...
        var domain = (cell1.find('.comhead').text().match(/\(\s?([^()]+)\s?\)/i) || [, null])[1] // eslint-disable-line
  ...
  if (!/[<>]/.test(body)) {
    ...
    try {
      ...
      if (cell1.length) {
        ...
        url = link.attr('href')
        domain = (cell1.find('.comhead').text().match(/\(\s?([^()]+)\s?\)/i) || [, null])[1] // eslint-disable-line
        ...
        /* eslint-disable */
        if (cell.length) {
          const userLink = cell.find('a[href^=user]')
          user = userLink.text(),
          timeAgo = userLink[0] ? userLink[0].nextSibling.textContent.replace('|', '').trim() : '',
...
exports.newComments = function (body, fn) {
  if (!/[<>]/.test(body)) {
    ...
    try {
      ...
      var commentRows = $('tr:nth-child(3) tr:has(.comment)')
